    ...
    def render(ruleName: String): Expr[RuleX] = reify {
      ...
      def continuation(): Rule0 = {
        ...
        while (ix < string.length && lastCharMatched) {
          lastChar = p.__nextChar()
          println(s"--> $lastChar ${string.charAt(ix)}")
          lastCharMatched = lastChar == string.charAt(ix)
          if (lastCharMatched) ix += 1
        }

        if (!lastCharMatched && lastChar != EOI) {
          println(s"LitStr $string - mismatched")
          p.__onCharMismatch()
          Rule.mismatched
        } else if (ix == string.length) {
          println(s"LitStr $string - matched")
          Rule.matched
        } else {
          println(s"LitStr $string - part matched")
    ...
    def render(ruleName: String): Expr[RuleX] = reify {
      ...
      def continuation(): Rule0 = {
        if (p.__nextChar() == char) {
          ...
          println(s"LiteralChar $char part matched")
          Rule.partiallyMatched(continuation)
        }
      }
      try continuation()
