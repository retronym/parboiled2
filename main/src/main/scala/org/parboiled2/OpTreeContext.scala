...
trait OpTreeContext[I <: HList, O <: HList, OpTreeCtx <: Parser.ParserContext] {
  val c: OpTreeCtx
  import c.universe._
  ...
  abstract class OpTree {
    def render(ruleName: String = ""): Expr[RuleX] @cpsParam[Rule[I, O], Nothing]
  }
  ...
  case class LiteralString(stringTree: Tree) extends OpTree {
    ...
    //      var ix = 0
    //      var charMatched = true
    //      while (ix < string.length && charMatched) {
    //        charMatched = p.__nextChar() == string.charAt(ix)
    //        if (charMatched) ix += 1
    //      }
    //      Rule(charMatched)
    //    }

    def render(ruleName: String): Expr[RuleX] @cpsParam[Rule[I, O], Nothing] = c.Expr[RuleX](q"""
      val p = ${c.prefix}
